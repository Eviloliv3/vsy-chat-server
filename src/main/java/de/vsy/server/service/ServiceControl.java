...
class ServiceControl {
     ...
     */
    public
    ServiceControl (final ServerDataManager serverData,
                    final ServerPersistentDataManager serverPersistentDataManager) {

        serviceDataModel = new ServiceDataAccessManager(serverData,
     ...
     */
    private
    void startService (final Service newService) {
        Set<Thread> sameTypeServices = this.registeredServices.computeIfAbsent(
                newService.getServiceType(), serviceType -> new HashSet<>());
        ...
        do {
            Thread.yield();
        } while (newServiceThread.isAlive() && !newService.getReadyState() &&
     ...
     */
    public
    boolean stopService (final Service.TYPE serviceType, final String threadName) {
    ...
    private
    void setupServiceDataManager (
            final ServiceDataAccessManager serviceDataManager) {
