...
public class ServiceControl {
	...
	private void startService(final Service newService) {
		Set<Thread> sameTypeServices = this.registeredServices.computeIfAbsent(newService.getServiceType(),
		...
		do {
			Thread.yield();
		} while (newServiceThread.isAlive() && !newService.getReadyState() && !Thread.currentThread().isInterrupted());
	}
