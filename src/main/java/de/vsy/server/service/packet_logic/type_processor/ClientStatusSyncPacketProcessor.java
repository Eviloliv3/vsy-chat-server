...
public class ClientStatusSyncPacketProcessor implements ServicePacketProcessor {
	 ...
	 * entfernten Server kommt. Der Server wird als synchronisiert auf den
	 * Paketdaten eingetragen. Zuletzt werden, sofern erforderlich, Klienten
	 * informiert.
	 *
	...
	private void translateState(final BaseStatusSyncDTO clientStatusData, final int serviceId) {
		final var remoteClientBuffer = this.serviceBufferManager.getSpecificBuffer(Service.TYPE.SERVER_TRANSFER,
				serviceId);
		final var subscriptions = createSubscriptionMap(clientStatusData);
		SubscriptionHandler subscriptionLogic = clientStatusData.isToAdd() ? this.clientSubscriptionManager::subscribe
	...
	private Map<PacketCategory, Set<Integer>> createSubscriptionMap(final BaseStatusSyncDTO statusSync) {
		final var clientState = statusSync.getClientState();
		final var clientId = statusSync.getContactData().getCommunicatorId();
		final var subscriptions = ClientStateTranslator.prepareClientSubscriptionMap(clientState, statusSync.isToAdd(),
				clientId);
