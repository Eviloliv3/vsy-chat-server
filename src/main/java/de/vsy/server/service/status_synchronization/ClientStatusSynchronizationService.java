...
public class ClientStatusSynchronizationService extends ServiceBase {
	...
	static {
		SERVICE_SPECIFICATIONS = ServiceData.ServiceDataBuilder.create().withType(Service.TYPE.CHAT_STATUS_UPDATE)
				.withName("ClientSyncService")
				.withDirection(ServiceData.ServiceResponseDirection.INBOUND, Service.TYPE.REQUEST_ROUTER)
				.withDirection(ServiceData.ServiceResponseDirection.OUTBOUND, Service.TYPE.SERVER_TRANSFER).build();
	}
	 ...
	 */
	public ClientStatusSynchronizationService(final ClientStatusRegistrationServiceDataProvider serviceDataModel) {
		...
		this.processor = new ServicePacketProcessor(
				new ClientStatusPacketProcessorFactory(resultingPackets, serviceDataModel), resultingPackets);
	...
	public void finishSetup() {
		...
		this.incomingBuffer = this.serviceBuffers.registerBuffer(SERVICE_SPECIFICATIONS.getServiceType(), serviceId);
