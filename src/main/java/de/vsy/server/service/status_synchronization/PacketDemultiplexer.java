...
public class PacketDemultiplexer {
		...
		if (toDemultiplex != null && eligibleRecipients != null && !eligibleRecipients.isEmpty()) {
			...
			if (updatePacket != null) {
				...
				for (final int contactId : eligibleRecipients) {
					...
					final var finalizedNotification = PacketCompiler.createRequest(currentRecipient, updatePacket);
	 ...
	 * @return the packet
	 */
	private static PacketContent createClientNotificationContent(final ExtendedStatusSyncDTO clientNotification) {
	...
	private static PacketContent prepareNotificationContent(final ExtendedStatusSyncDTO clientNotification) {
		final PacketContent clientStatusData;

		if (clientNotification.getClientState() == ClientState.ACTIVE_MESSENGER) {
			clientStatusData = new ContactMessengerStatusDTO(EligibleContactEntity.CLIENT, clientNotification.isToAdd(),
