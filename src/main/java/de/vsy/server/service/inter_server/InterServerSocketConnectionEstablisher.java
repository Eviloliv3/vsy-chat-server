...
public class InterServerSocketConnectionEstablisher implements InterServerCommunicationServiceCreator {
	...
	public InterServerSocketConnectionEstablisher(final ServerConnectionDataManager serverConnectionDataManager,
	...
	private void setupServerSocket() {
		...
		do {
			...
			try {
				...
				LOGGER.error("InterServerSocket konnte nicht auf " + "Port {} geoeffnet werden.", masterSocketPort);
			}
		} while (this.localMasterSocket == null);
		serverReceptionConnection = LocalServerConnectionData.valueOf(masterSocketPort, this.localMasterSocket);
		...
		for (int test = 1, maxRunningServers = ConnectionSpecifications.getServerports()
			...
			if (testPort != localMasterPort) {
				...
				try {
					...
					LOGGER.error("Es konnte keine Verbindung zu entferntem " + "Server aufgebaut werden. {}:{}",
							hostname, testPort);
				}
			}
		}
	...
	@Override
	public void createInterServerService(final boolean isLeader, final Socket remoteServerConnection) {
