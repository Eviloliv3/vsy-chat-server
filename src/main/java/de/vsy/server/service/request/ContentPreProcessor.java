...
public class ContentPreProcessor implements PublishablePacketCreator {
	...
	public ContentPreProcessor(final LocalServerConnectionData localServerData,
			AbstractPacketCategorySubscriptionManager clientSubscriptions, OutputBuffer assignmentBuffer) {
		this.localServerData = localServerData;
		extendedStatusProcessor = new ExtendedClientStatusPreProcessor(clientSubscriptions, assignmentBuffer);
	}
	...
	public Packet handleDistributableContent(Packet input) throws PacketProcessingException {
		...
		if (isLocalBroadcast(input) && input.getPacketContent() instanceof ExtendedStatusSyncDTO statusContent) {
