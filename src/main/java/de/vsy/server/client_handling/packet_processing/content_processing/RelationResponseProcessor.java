...
public class RelationResponseProcessor implements ContentProcessor<ContactRelationResponseDTO> {
	...
	@Override
	public void processContent(ContactRelationResponseDTO extractedContent) throws PacketProcessingException {
		...
		if (isFriendshipRequest && extractedContent.getDecision()) {
			RelationManipulator.addContact(requestData.getContactType(), contactId, this.contactListAccess);
			this.appendStatusMessage(contactId, true);
		} else {
			if (iAmOriginator && !isFriendshipRequest) {
				RelationManipulator.removeContact(requestData.getContactType(), contactId, this.contactListAccess,
	 ...
	 */
	private void checkResponseLegitimacy(final ContactRelationResponseDTO contactResponse, final int contactId)
			throws PacketProcessingException {
		final var requestData = contactResponse.getRequestData();
		final var recipientId = requestData.getRecipientId();
		final var desiredState = requestData.getDesiredState();
		final var contactsAlready = this.contactListAccess.checkAcquaintance(requestData.getContactType(), recipientId);

		if (desiredState && contactsAlready) {
			final var contactData = this.threadDataAccess.getContactToActiveClientMapper().getContactData(contactId);
			throw new PacketProcessingException("Freundschaftsanfrage wurde nicht " + "verarbeitet. Sie sind bereits "
	...
	private void appendStatusMessage(final int contactId, boolean contactAdded) {
		...
		final var contactData = threadDataAccess.getContactToActiveClientMapper().getContactData(contactId);
		final var contactDTO = ConvertCommDataToDTO.convertFrom(contactData);

		contactToAdd = !checkContactOnline(contactId) && contactAdded;
		contactStatusContent = new ContactMessengerStatusDTO(EligibleContactEntity.CLIENT, contactToAdd, contactDTO,
	...
	private boolean checkContactOnline(final int contactId) {
		return !(this.threadDataAccess.getContactToActiveClientMapper().removeOfflineContacts(Set.of(contactId))
				.isEmpty());
	}
}
