...
public
class StandardProcessorFactoryProvider
        implements CategoryBasedProcessorFactoryProvider {
    ...
    ContentBasedProcessorFactory getCategoryHandlerFactory (PacketCategory category,
        ...
        switch (category) {
            case AUTHENTICATION ->
                    categoryFactory = new AuthenticationPacketProcessorFactory(
                            threadDataAccess);
            case CHAT -> categoryFactory = new ChatPacketProcessorFactory(
                    threadDataAccess);
            case STATUS -> categoryFactory = new StatusPacketProcessorFactory(
                    threadDataAccess);
            case RELATION -> categoryFactory = new RelationPacketProcessorFactory(
                    threadDataAccess);
            case ERROR -> categoryFactory = new ErrorPacketProcessorFactory(
                    threadDataAccess);
            default -> {
            }
            // fehler loggen
