...
public class ClientStatusChangeProcessor implements ContentProcessor<ClientStatusChangeDTO> {
	...
	public ClientStatusChangeProcessor(final StatusHandlingDataProvider threadDataAccess) {
		...
		this.contactProvider = threadDataAccess.getLocalClientStateDependentLogicProvider().getClientPersistentAccess()
				.getContactlistDAO();
		this.messageReader = threadDataAccess.getLocalClientStateDependentLogicProvider().getClientPersistentAccess()
				.getMessageDAO();
		this.contentHandler = threadDataAccess.getResultingPacketContentHandler();
	}

	@Override
	public void processContent(ClientStatusChangeDTO validatedContent) throws PacketProcessingException {
		...
		if (this.clientStateManager.changeClientState(ClientState.ACTIVE_MESSENGER, changeTo)) {
			...
			if (this.clientStateManager.changePersistentClientState(ClientState.ACTIVE_MESSENGER, changeTo)) {
			...
			causeMessage = "Zustand konnte nicht angepasst werden. Sie sind bereits " + "im gewünschten Zustand.";
		}
