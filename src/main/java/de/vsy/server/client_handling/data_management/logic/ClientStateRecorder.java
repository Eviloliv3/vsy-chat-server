...
public class ClientStateRecorder implements AuthenticationStateControl {
	...
	public static void setupStaticServerDataAccess(final LiveClientStateDAO persistentClientStateAccess,
	...
	@Override
	public boolean changePersistentClientState(final ClientState clientState, final boolean changeTo) {
		final var clientId = this.localClientDataManager.getClientId();

		if (clientState.equals(ClientState.AUTHENTICATED) && !changeTo) {
			return persistentClientStates.removeClientState(clientId);
		} else {
			return persistentClientStates.changeClientState(serverNode.getServerId(), clientId, clientState);
		}
	}
}
