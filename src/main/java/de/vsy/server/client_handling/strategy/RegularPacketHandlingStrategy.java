...
import de.vsy.shared_module.shared_module.packet_validation.content_validation.ClientPacketTypeValidationCreator;
import de.vsy.shared_transmission.shared_transmission.packet.Packet;
import de.vsy.shared_transmission.shared_transmission.packet.content.error.ErrorDTO;

/**
 * Contains Packetprocessing strategies during live connection. Pending Packet
 * are added to the respective Buffers. Neither remaining Packet nor pending
 * Packet are dealt with.
 */
public class RegularPacketHandlingStrategy implements PacketHandlingStrategy {
	...
	public RegularPacketHandlingStrategy(final HandlerLocalDataManager threadDataAccess,
		...
		this.persistentData = threadDataAccess.getLocalClientStateDependentLogicProvider().getClientPersistentAccess();
	...
	private void setupProcessor(final HandlerLocalDataManager threadDataAccess) {
		...
		final var contextCheckLink = new PacketContextCheckLink(processorLink,
				threadDataAccess.getLocalClientStateDependentLogicProvider().getPermittedPacketCategoryCheck());
		this.processor = new PacketSyntaxCheckLink(contextCheckLink, new SimplePacketChecker(setupValidator()));
	}
	...
	private SemanticPacketValidator setupValidator() {
		...
		for (var categoryValidator : regularServerValidation.entrySet()) {
			packetValidator.addCategoryAssociations(categoryValidator.getKey(), categoryValidator.getValue());
		}
