...
/**
 * Handles client connection as well as processing requests and transferring
 * them into server readable structures if needed .
 */
public class ClientConnectionHandler implements Runnable {
	 ...
	 */
	public ClientConnectionHandler(final Socket clientSocket, final PacketBuffer requestAssignmentBuffer) {
	...
	public void run() {
		...
		if (this.connectionControl.initiateConnectionThreads()) {
			clientHandling = new RegularPacketHandlingStrategy(this.threadDataManager, this.connectionControl);
	...
	private void finishThreadSetup() {
		...
		this.connectionControl = new ConnectionThreadControl(this.clientConnection,
				this.threadDataManager.getHandlerBufferManager(), new UnconfirmedPacketTransmissionCache(1000), true);
	}
	...
	private void saveClientBoundPackets() {
		...
		if (pendingPacketAccessor != null) {
			...
			while (!notReceivedPackets.isEmpty()) {
				pendingPacketAccessor.appendPendingPacket(PendingType.CLIENT_BOUND, notReceivedPackets.poll());
			}
