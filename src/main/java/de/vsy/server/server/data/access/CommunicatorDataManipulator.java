...
/**
 * Provides connected client thread's accessLimiter to other clients
 * dataManagement.
 */
public class CommunicatorDataManipulator {
	 ...
	 */
	public CommunicatorData createNewAccount(final String loginName, final String password, final String displayName) {
		...
		if (this.clientAuthPersist.saveAccountData(authData)) {
			...
			if (!this.communicatorDataPersist.addCommunicator(communicatorData)) {
				...
				LOGGER.error("Kommunikatordaten konnten nicht " + "gespeichert werden. Authentifikationsdaten "
	...
	/**
	 * Erstellt einen neuen Kommunikator, sofern eine gültige
	 * Besitzer-(/Klienten-)Id angegeben wurde.
	 *
	...
	public CommunicatorData createNewGroup(final int ownerId, final String groupName) {
		...
		if (this.clientAuthPersist.checkClientId(ownerId)) {
			...
			if (!this.communicatorDataPersist.addCommunicator(communicatorData)) {
				...
				LOGGER.error("Gruppe \"{}:{}\"wurde nicht erstellt. " + "Es gibt bereits gleichnamige Gruppe", ownerId,
						groupName);
			}
		} else {
			LOGGER.error("Gruppe \"{}:{}\"wurde nicht erstellt. Die " + "Klienten-Id ist ungültig.", ownerId,
					groupName);
		}
		return communicatorData;
	}
