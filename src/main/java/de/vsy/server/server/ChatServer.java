...
public class ChatServer implements ClientServer {
	...
	public boolean prepareServer() {
		...
		if (!serverPrepared) {
			LOGGER.warn(
					"Keiner der vorgegebenen Ports ist mehr frei. Der Server " + "kann somit nicht gestartet werden.");
			return false;
		} else {
	...
	public void shutdownServer() {
		LOGGER.info("Server wird heruntergefahren. Unterbrechungsstatus: {}", Thread.currentThread().isInterrupted());
	...
	private void setupStaticServerDataAccess() {
		...
		PacketCompiler.addOriginatorEntityProvider(
				() -> getServerEntity(serverConnectionDataManager.getLocalServerConnectionData().getServerId()));
		...
		ClientStateRecorder.setupStaticServerDataAccess(this.serverPersistentDataManager.getClientStateAccessManager(),
				serverConnectionDataManager.getLocalServerConnectionData());
	}

	private void setupAndStartServices() {
		this.serviceControl = new ServiceControl(this.serverDataModel, this.serverPersistentDataManager);
	...
	public void serveClient(Socket clientConnectionSocket) {
		...
		if (clientConnectionSocket != null) {
			...
			this.connectionPool.execute(new ClientConnectionHandler(clientConnectionSocket, requestAssignmentBuffer));
		} else {
