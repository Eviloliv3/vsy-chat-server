...
/**
 * Grants writing accessLimiter to the file containing a client's pending
 * Packet.
 */
public class PendingPacketDAO implements ClientDataAccess, PendingPacketPersistence {
	...
	public static JavaType getDataFormat() {
		...
		final var linkedPendingMap = factory.constructMapType(LinkedHashMap.class, String.class, Packet.class);
	...
	public boolean appendPendingPacket(final PendingType classification, final Packet toAppend) {
		if (classification == null || toAppend == null) {
			throw new IllegalArgumentException("Ungueltiger Parameter. " + classification + "/" + toAppend);
		}
	 ...
	 * @return true, if successful
	 */
	public boolean setPendingPackets(final PendingType classification, final Map<String, Packet> toSet) {
		var packetAdded = false;
		Map<PendingType, LinkedHashMap<String, Packet>> allPendingPackets;
