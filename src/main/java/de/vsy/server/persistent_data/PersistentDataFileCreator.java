...
/**
 * Simple tool for creating preset file types. Assuming the passed directories have
 * already been created. fredward
 */
public
class PersistentDataFileCreator {

    /** The Enum DataFileDescriptor. */
    public
    enum DataFileDescriptor implements RequirementProvider {
        /* The active clients */
        ACTIVE_CLIENTS(false, "activeClientsUTF_8.json", "activeClients.lock"),
        CLIENT_TRANSACTION(false, "clientTransactionsUTF_8.json",
                           "clientTransactions.lock"),
        COMMUNICATORS(false, "communicatorsUTF_8.json", "communicatorData.lock"),
        CONTACT_LIST(true, "contactListUTF_8.json", "contactList.lock"),
        ID_MAP(false, "IdMapUTF_8.json", "IdMap.lock"),
     ...
     */
    public static
    Path[] createAndGetFilePaths (final String[] directoryNames,
                                  final String filename, final Logger LOGGER) {
        Path[] files;
        final int fileCount = directoryNames.length;
        files = new Path[fileCount];

        for (var i = 0; i < fileCount; i++) {
            final Path currentReference = createAndGetFilePath(directoryNames[i],
                                                               filename, LOGGER);
    ...
    public static
    Path createAndGetFilePath (final String directoryName, final String filename,
                               final Logger LOGGER) {
        ...
        try {
            ...
            if (directory.isDirectory()) {
                ...
                if (!file.isFile()) {
                    ...
                    if (!newFileIsCreated) {
                        LOGGER.error("Datei {} sollte neu erstellt " +
            ...
            LOGGER.error(
                    "Datei (Pfad: {}: {}) durfte nicht erstellt werden. (SecurityException)",
                    directoryName, filename);
            fileReference = null;
        } catch (final IOException ex) {
            Thread.currentThread().interrupt();
            LOGGER.error(
                    "Datei (Pfad: {}: {}) konnte nicht erstellt werden. (IOException)\n{}",
                    directoryName, filename, asList(ex.getStackTrace()));
            fileReference = null;
        }
