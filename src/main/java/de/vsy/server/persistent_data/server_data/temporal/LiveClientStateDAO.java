...
public class LiveClientStateDAO implements ServerDataAccess {
	...
	public static JavaType getDataFormat() {
		return defaultInstance().constructMapType(HashMap.class, Integer.class, CurrentClientState.class);
	}

	public Map<PacketCategory, Set<Integer>> getAllExtraSubscriptions(final int clientId) {
		final Map<PacketCategory, Set<Integer>> extraSubscriptions = new EnumMap<>(PacketCategory.class);
	 ...
	 */
	public boolean changeClientState(final int serverPort, final int clientId, final ClientState newState) {
	...
	public boolean addExtraSubscription(final int clientId, PacketCategory topic, final int threadId) {
		CurrentClientState clientState;
		Map<Integer, CurrentClientState> clientStateMap;
		...
		if (clientState != null) {
			...
			LOGGER.info("{} - Zustand wird nicht verwaltet: Zusatzabonnement nicht hinzugefügt.", clientId);
		}
	...
	public boolean removeExtraSubscription(final int clientId, PacketCategory topic, final int threadId) {
		CurrentClientState clientState;
		Map<Integer, CurrentClientState> clientStateMap;
		var subscriptionAdded = false;
		if (!this.dataProvider.acquireAccess(true)) {
	...
	public boolean changeClientPendingState(final int clientId, final boolean pendingState) {
		...
		if (clientState != null) {
			...
			LOGGER.info("{} - Zustand wird nicht verwaltet: PendingState wurde nicht geändert.", clientId);
		}
	...
	public boolean removeClientState(final int clientId) {
		...
		clientStateMap = getAllActiveClientStates();
		clientStateRemoved = (clientStateMap.remove(clientId) != null) && this.dataProvider.writeData(clientStateMap);
		this.dataProvider.releaseAccess(true);
