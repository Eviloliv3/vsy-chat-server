...
import de.vsy.chat.server.client_handling.data_management.bean.ClientDataManager;
...
public
class ClientStateRecorder implements AuthenticationStateControl {
    ...
    public
    ClientStateRecorder (final ClientStateManager localClientStateManager,
    ...
    void logoutClient () {
        ...
        for (int stateIndex = (statesToRemove.size() - 1);
             stateIndex >= 0;
             stateIndex--) {
    ...
    @Override
    public
    boolean changePersistentClientState (final ClientState clientState,
                                         final boolean changeTo) {
        final var clientId = this.localClientDataManager.getClientId();
        ...
        if (clientState.equals(ClientState.AUTHENTICATED) && !changeTo) {
            ...
            return persistentClientStates.changeClientState(serverNode.getServerId(),
