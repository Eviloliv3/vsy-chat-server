...
import de.vsy.chat.server.client_handling.data_management.access_limiter.ChatHandlingDataProvider;
...
import de.vsy.chat.shared_utility.id_manipulation.IdComparator;
import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;

public
class TextMessageProcessor implements ContentProcessor<TextMessageDTO> {
     ...
     */
    public
    TextMessageProcessor (final ChatHandlingDataProvider threadDataAccess) {
    ...
    @Override
    public
    void processContent (TextMessageDTO extractedContent)
    throws PacketProcessingException {
        ...
        if (messageToReturn) {
            this.resultingPacketCache.addResponse(
                    extractedContent.setReceptionState());
        }
    }

    private
    void checkLegitimacy (final TextMessageDTO textMessage)
    throws PacketProcessingException {
        final var contactId = IdComparator.determineContactId(
                localClientData.getClientId(), textMessage.getOriginatorId(),
     ...
     */
    private
    boolean saveMessage (final TextMessageDTO messageData) {
        final var recipientId = messageData.getRecipientId();

        final var contactId = recipientId ==
                              this.localClientData.getClientId() ? messageData.getOriginatorId() : recipientId;
