...
public
class ExtraClientSubscriptionProvider {

    private final ContactListDAO contactListAccessor;

    public
    ExtraClientSubscriptionProvider (final ContactListDAO contactListAccessor) {
        this.contactListAccessor = contactListAccessor;
    }

    public
    Map<PacketCategory, Set<Integer>> getExtraSubscriptionsForState (
            final ClientState clientState) {
        Map<PacketCategory, Set<Integer>> extraSubscriptions = new EnumMap<>(
                PacketCategory.class);
        Set<Integer> threadList;

        if (ClientState.ACTIVE_MESSENGER.equals(clientState)) {
            threadList = new HashSet<>(this.contactListAccessor.readContacts(
