 ...
 *
 */
package de.vsy.chat.server.client_handling;
...
import de.vsy.chat.server.client_handling.data_management.bean.ClientStateManager;
import de.vsy.chat.server.client_handling.strategy.PacketHandlingStrategy;
import de.vsy.chat.server.client_handling.strategy.PendingChatPacketHandling;
import de.vsy.chat.server.client_handling.strategy.RegularPacketHandlingStrategy;
...
class ClientConnectionHandler implements Runnable {
     ...
     */
    public
    ClientConnectionHandler (final Socket clientSocket,
                             final PacketBuffer requestAssignmentBuffer) {
        this.clientConnection = clientSocket;
        this.threadDataManager = new HandlerLocalDataManager(
    ...
    void run () {
        ...
        if (this.connectionControl.initiateConnectionThreads()) {
            clientHandling = new RegularPacketHandlingStrategy(
                    this.threadDataManager, this.connectionControl);
            ...
            if (stateManager.checkClientState(ClientState.AUTHENTICATED)) {
                LOGGER.info(
                        "Klient hat sich nicht abgemeldet und wird in schwebenden Zustand versetzt.");
    ...
    void finishThreadSetup () {
        ...
        this.connectionControl = new ConnectionThreadControl(this.clientConnection,
                                                             ...
                                                             new UnconfirmedPacketTransmissionCache(
                                                                     1000), true);
    }

    /**
     * Removes Packets from UnconfirmedPacketTransmissionCache and then CLIENT_BOUND
     * buffer. Saves them in with PendingPacketDAO.
     */
    private
    void saveClientBoundPackets () {
        ...
        if (pendingPacketAccessor != null) {
            ...
            while (handlerBuffer.containsPackets()) {
                try {
                    ...
                    pendingPacketAccessor.appendPendingPacket(
                            PendingType.CLIENT_BOUND, currentPacket);
                } catch (InterruptedException e) {
                    reinterrupt = true;
                    LOGGER.error("Buffer wird geleert. Interrupt wird " +
                                 "zeitweise ignoriert.");
                }
            }
