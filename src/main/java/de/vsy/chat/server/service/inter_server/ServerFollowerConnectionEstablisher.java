...
public
class ServerFollowerConnectionEstablisher extends ThreadContextRunnable {
    ...
    public
    ServerFollowerConnectionEstablisher (
    ...
    public
    Socket acceptFollowerConnection (final ServerSocket socketToWatch) {
        Socket followerSocket = null;

        try (socketToWatch) {
            final var futureFollower = this.acceptingThread.submit(
                    socketToWatch::accept);
            followerSocket = futureFollower.get();
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            LOGGER.info("Verbindungsaufnahme mit Follower-Server wurde " +
                        "unterbrochen.");
        } catch (IOException ioe) {
            Thread.currentThread().interrupt();
            LOGGER.error(
                    "Fehler bei der Verbindungsaufnahme mit Follower-Servern. Fehlernachricht:\n{}",
                    ioe.getMessage());
