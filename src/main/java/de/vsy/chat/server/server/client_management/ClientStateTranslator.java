...
class ClientStateTranslator {
    ...
    static {
        ...
        CLIENT_STATE_TO_CATEGORY_SUBSCRIPTIONS.put(ClientState.AUTHENTICATED,
                                                   of(STATUS, ERROR, RELATION));
        CLIENT_STATE_TO_CATEGORY_SUBSCRIPTIONS.put(ClientState.ACTIVE_MESSENGER,
                                                   of(CHAT));
    }

    /** Instantiates a new state to subscription translator. */
    private
    ClientStateTranslator () {
    }
     ...
     */
    public static
    Map<PacketCategory, Set<Integer>> prepareClientSubscriptionMap (
            final ClientState clientState, final boolean onlineStatus,
        ...
        if (dependingStateProvider != null) {
            final var dependingStates = dependingStateProvider.getDependencies(
                    onlineStatus);
    ...
    private static
    Map<PacketCategory, Set<Integer>> createTopicThreadMap (final int communicatorId,
                                                            Set<ClientState> clientStateSet) {
        final var topicThreadMap = new EnumMap<PacketCategory, Set<Integer>>(
                PacketCategory.class);

        for (final var currentState : clientStateSet) {
            final var topics = CLIENT_STATE_TO_CATEGORY_SUBSCRIPTIONS.get(
                    currentState);
