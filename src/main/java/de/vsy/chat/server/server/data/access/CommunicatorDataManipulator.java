 ...
 */
public
class CommunicatorDataManipulator {
     ...
     */
    public
    CommunicatorDataManipulator (final ServerPersistentDataManager persistenceDAO) {
     ...
     */
    public
    CommunicatorData createNewAccount (final String loginName, final String password,
        ...
        if (this.clientAuthPersist.saveAccountData(authData)) {
            communicatorData = CommunicatorData.valueOf(clientId, clientId,
                                                        displayName);
            ...
            if (!this.communicatorDataPersist.addCommunicator(communicatorData)) {
                ...
                LOGGER.error("Kommunikatordaten konnten nicht " +
                             "gespeichert werden. Authentifikationsdaten " +
            ...
            LOGGER.error("Authentifizierungsdaten konnten nicht " +
                         "gespeichert werden.");
        }
    ...
    /**
     * Erstellt einen neuen Kommunikator, sofern eine gültige Besitzer-(/Klienten-)Id
     * angegeben wurde.
     *
     ...
     */
    public
    CommunicatorData createNewGroup (final int ownerId, final String groupName) {
        CommunicatorData communicatorData = null;
        ...
        if (this.clientAuthPersist.checkClientId(ownerId)) {
            ...
            if (!this.communicatorDataPersist.addCommunicator(communicatorData)) {
                ...
                LOGGER.error("Gruppe \"{}:{}\"wurde nicht erstellt. " +
                             "Es gibt bereits gleichnamige Gruppe", ownerId,
                             groupName);
            }
        } else {
            LOGGER.error("Gruppe \"{}:{}\"wurde nicht erstellt. Die " +
                         "Klienten-Id ist ungültig.", ownerId, groupName);
        }
        return communicatorData;
    }
     ...
     */
    public
    boolean deleteAccount (final int clientId) {
    ...
    CommunicatorData getCommunicatorData (final int communicatorId) {
        ...
        try {
            ...
            foundCommunicatorData = this.communicatorDataPersist.getCommunicatorData(
                    communicatorId);
        } finally {
     ...
     */
    public
    CommunicatorData getCommunicatorData (final CommunicatorDTO communicatorData) {
        ...
        if (communicatorData != null) {
            ...
            try {
                ...
                if (foundCommunicatorData != null &&
                    !foundCommunicatorData.getDisplayName()
                                          .equals(communicatorData.getDisplayLabel())) {
     ...
     */
    public
    CommunicatorData getCommunicatorData (final String loginName,
        ...
        try {
            ...
            if (loginName != null && password != null) {
                final var clientId = this.clientAuthPersist.getClientId(loginName,
                                                                        password);
                communicatorData = this.communicatorDataPersist.getCommunicatorData(
                        clientId);
            }
        } finally {
            this.lock.readLock().unlock();
