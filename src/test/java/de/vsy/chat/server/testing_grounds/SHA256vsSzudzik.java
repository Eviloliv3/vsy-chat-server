...
class SHA256vsSzudzik {
  ...
  void timingChange() throws NoSuchAlgorithmException {
    ...
    LOGGER.info("SHA komplett neu braucht: {} Sekunden",
        MICROSECONDS.convert(overalltime, NANOSECONDS));
  ...
  void testSHA256() throws NoSuchAlgorithmException {
    ...
    LOGGER.info("SHA braucht: {}",
        MICROSECONDS.convert((endtime - starttime), NANOSECONDS) + " Sekunden");
    ...
    endtime = nanoTime();
    LOGGER.info("Gesamtzeit: {}",
        TimeUnit.NANOSECONDS.convert((endtime - starttime), TimeUnit.MILLISECONDS));
  }
}
