...
class TestClientMessage extends ServerTestBase {
    ...
    public
    TestClientMessage (final ServerPortProvider clientConnectionPorts,
...
/*
    ...
    throws IOException {
        ...
        final var inactiveClientId = clientTwo.getCommunicatorData()
                                              .getCommunicatorId();
 ...
 */
    ...
    private
    ClientConnection loginSpecificClient (final AuthenticationDTO credentials) {
    ...
    throws IOException {
        ...
        if (sentPacket != null && receivedPacket != null && responsePacket != null) {
            if (sentPacket.getPacketContent() instanceof TextMessageDTO textMessage &&
                ...
                Assertions.assertEquals(textMessage.getMessage(),
                                        receivedMessage.getMessage(),
                                        responseMessage.getMessage());
            } else {
                Assertions.fail("Fehlerhafter Paketinhalt:\nGesandt: " +
                                sentPacket.getPacketContent() + "\nEmpfangen: " +
                                receivedPacket.getPacketContent() + "\nAntwort: " +
                                responsePacket.getPacketContent());
            }
        } else {
            Assertions.fail(
                    "Eines der erwarteten Pakete fehlt:\nGesandt: " + sentPacket +
                    "\nEmpfangen: " + receivedPacket + "\nAntwort: " +
                    responsePacket);
        }
    }
