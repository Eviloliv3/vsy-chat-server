...
class ServerTestBase {
     ...
     * subsequently be gotten hold of using the <ref>getUnusedClientConnection()
     * </ref>-method. The connection object will still need to be setup
     * subsequently. If there was only one server port specified, the connection will
    ...
    @AfterEach
    protected
    void breakDownServerConnection ()
    ...
    protected
    void logoutClient (final ClientConnection toLogout) {
        ...
        if (logoutSuccess) {
            ...
            this.activeClientAuthenticationData.remove(
                    toLogout.getAuthenticationData());
        } else {
    ...
    /**
     * Tries to get an unused set of login credentials and will proceed with the
     * login process by calling <ref>loginNextClient(AuthenticationDTO)</ref>, if a
     * set of unused login credentials was found. Also requires a setup unused
     * ClientConnection object. That can be created calling one of the
     * addConnection****Server()methods
     *
     * @return the ClientConnection with logged in client, or null if no unused
     *         credentials or no setup, unused ClientConnection object could be
     *         found.
     */
    protected
    ClientConnection loginNextClient () {
        ...
        if (clientAuthenticationData != null) {
            ...
            LOGGER.error("Login-Versuch fehlgeschlagen. Keine gueltigen " +
                         "Logindaten gefunden.");
        }
        return connection;
    }
    ...
    protected
    AuthenticationDTO getUnusedAuthenticationData () {

        for (var currentAuthenticationData : this.clientAuthenticationDataList) {
            if (!this.activeClientAuthenticationData.contains(
    ...
    protected
    ClientConnection loginNextClient (AuthenticationDTO clientAuthenticationData) {
        if (clientAuthenticationData == null) {
            throw new IllegalArgumentException(
                    "Keine Authentifizierungsdaten " + "erstellt.");
        }
        ...
        if (clientConnection != null) {
            ...
            LOGGER.error("Login-Versuch fehlgeschlagen. Keine nutzbare " +
                         "Verbindung gefunden.");
        }
    ...
    protected
    ClientConnection getUnusedClientConnection () {
