...
class TestStatusChange extends ServerTestBase {
    ...
    public
    TestStatusChange (final ServerPortProvider clientConnectionPorts,
    ...
    throws IOException {
        LOGGER.info(
                "Test: Status zurücknehmen für mehrere Klienten -> erfolgreich -- gestartet");
        Packet packet;
        ...
        content = new ClientStatusChangeDTO(MESSENGER, true,
        ...
        content = new ClientStatusChangeDTO(MESSENGER, true,
                                            clientTwo.getCommunicatorData());
        checkResponse(clientTwo, getServerEntity(STANDARD_SERVER_ID), content,
                      MessengerSetupDTO.class);
        ...
        if (packet != null &&
            ...
            Assertions.assertEquals(contactStatus.getContactData()
                                                 .equals(clientOne.getCommunicatorData()),
                                    !contactStatus.getOnlineStatus());
        } else {
            Assertions.fail(ContactMessengerStatusDTO.class.getSimpleName() +
                            " erwartet, erhalten: " + packet);
        }
    ...
    void changeFromMessengerStateSingleClientSuccess () {
        LOGGER.info(
                "Test: Status für einzelnen Klienten aufgehoben -> erfolgreich -- gestartet");
        Packet packet;
        PacketContent content;
        ...
        if (packet != null) {
            content = new ClientStatusChangeDTO(MESSENGER, false,
                                                clientOne.getCommunicatorData());
            clientOne.sendRequest(content, getServerEntity(STANDARD_SERVER_ID));
            packet = clientOne.readPacket();
    ...
    void changeToMessengerStateSingleClientSuccess () {
        LOGGER.info(
                "Test: Status für einzelnen Klienten gesetzt -> erfolgreich -- gestartet");
        PacketContent content;
    ...
    void changeMessengerNoStateFailed () {
        LOGGER.info(
                "Test: Status nicht erreichbar -> kein Status angegeben -- gestartet");
        ClientStatusChangeDTO content;
        final var clientOne = super.loginNextClient();

        content = new ClientStatusChangeDTO(null, true,
                                            clientOne.getCommunicatorData());
    ...
    throws IOException {
        LOGGER.info(
                "Test: Status für mehrere Klienten gesetzt -> erfolgreich -- gestartet");
        Packet packet;
        PacketContent content;
        final ClientConnection clientOne, clientTwo;
