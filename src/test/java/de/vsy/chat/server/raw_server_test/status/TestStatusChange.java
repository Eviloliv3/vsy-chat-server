...
public class TestStatusChange extends ServerTestBase {
	...
	public void changeFromMessengerStateTwoClientSuccess() throws IOException {
		...
		if (packet != null && packet.getPacketContent() instanceof ContactMessengerStatusDTO contactStatus) {
			Assertions.assertEquals(contactStatus.getContactData().equals(clientOne.getCommunicatorData()),
					!contactStatus.getOnlineStatus());
		} else {
			Assertions.fail(ContactMessengerStatusDTO.class.getSimpleName() + " erwartet, erhalten: " + packet);
		}
