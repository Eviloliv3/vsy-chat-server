...
class ClientConnectionManager {
    ...
    public
    ClientConnectionManager (final ServerPortProvider serverPorts) {
        this.serverPorts = serverPorts;
        this.clientConnectionList = new ArrayList<>();
    }

    public
    ClientConnection getConnection (int clientNumber) {
        ClientConnection foundClient = null;
        final var validIndex = (clientNumber < this.clientConnectionList.size()) &&
                               (clientNumber >= 0);
    ...
    public
    ClientConnection getUnusedConnection () {
        ClientConnection foundConnection = null;

        for (final var currentConnection : this.clientConnectionList) {
            if (currentConnection.getCommunicatorData().getCommunicatorId() ==
                STANDARD_CLIENT_ID) {
