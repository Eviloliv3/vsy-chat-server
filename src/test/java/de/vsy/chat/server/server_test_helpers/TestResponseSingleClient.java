...
public class TestResponseSingleClient {
	 ...
	 */
	public static void checkResponse(ClientConnection clientOne, CommunicationEndpoint recipient, PacketContent request,
		...
		if (packet != null) {
			...
			Assertions.assertTrue(expectedResponseType.isInstance(content), () -> "Antworttyp ist \"" + content + "\"."
					+ "\nErwartet wurde \"" + expectedResponseType.getSimpleName() + "\"");
		} else {
			Assertions.fail("Keine Antwort, an Stelle von \"" + expectedResponseType.getSimpleName() + "\" erhalten.");
		}
	}
